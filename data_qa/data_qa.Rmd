...
```{r}
custody_inmates_long <- stack_mft(custody_jurisdiction_inmates_stats)
```
```{r}
##Total number of inmates under your custody##

# ggplot(custody_inmates_long, aes(YEAR, CUSTOT, color=gender)) + 
#   geom_point(size=0.05) + 
#   #stat_smooth() +
#   facet_wrap(~state_name,scales = "free_y")+
#   theme(axis.text.x = element_text(angle = 90))

ggplot(custody_inmates_long, aes(YEAR, PVIN, color=gender)) + 
  geom_point(size=0.05) + 
  #stat_smooth() +
  facet_wrap(~state_name,scales = "free_y")+
  theme(axis.text.x = element_text(angle = 90))
...
ggplot(custody_inmates_long, aes(YEAR, JURTOT, color=gender)) + 
  geom_point(size=0.05) + 
  #stat_smooth() +
  facet_wrap(~state_name,scales = "free_y")+
  theme(axis.text.x = element_text(angle = 90))
...
library(corrplot)
library(corrgram)
df_final_vari <- df_final[ - as.numeric(which(apply(df_final, 2, var) == 0))]
apply(df_final, 2, var)
#a<-corrgram(facility_info)
corrplot(corrgram(custody_jurisdiction_inmates_stats),xlim=inf, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
library("PerformanceAnalytics")
#my_data <- mtcars[, c(1,3,4,5,6,7)]
my_data <- dplyr::select_if(df_final, is.numeric)
...
custody_jurisdiction_inmates_stats %>% 
  summarise_all(funs(var), na.rm = TRUE)
a<- custody_inmates_long %>% select(-c('STATEID','STATE','REGION','LFCRST','LFCRST','LFCRINC','LFCRINC'))
corrplot(corrgram(a),type = "upper")
...
facility_long %>% 
  summarise_all(funs(var), na.rm = TRUE)
a<- facility_long %>% select(-c('STATEID','STATE','REGION','state_name'))
corrplot(corrgram(a),type = "upper")
```
...
```{r}
library(ggplot2)
custody_jurisdiction_inmates_stats$Population


custody_jurisdiction_inmates_stats
ggplot(custody_jurisdiction_inmates_stats, aes(YEAR, Population)) + 
  geom_point() + 
  stat_smooth() +
  facet_wrap(~STATE)

ggplot(custody_jurisdiction_inmates_stats, aes(YEAR, CUSTOTM)) + 
  geom_point() + 
  stat_smooth() +
  facet_wrap(~STATE, scales = "free_y")

View(custody_jurisdiction_inmates_stats)
```
